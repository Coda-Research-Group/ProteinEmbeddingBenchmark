...
def get_protein_tm_score(path, protein_x, output):
    ...
    for protein_y in proteins:
        result = subprocess.run(
            [US_ALIGN, os.path.join(path, protein_x), os.path.join(path, protein_y)],
            capture_output=True,
        )
    ...
    for protein_tar, proteins in tqdm(protein_dict.items(), desc="Extracting proteins"):
        ...
        subprocess.run(
            [
                "tar",
                "-xf",
                os.path.join(PROTEIN_PATH, protein_tar),
                "-C",
                os.path.join(SCRATCH_DIR, protein_foldername),
            ]
        )
        proteins_to_unzip = [
        ...
        if not all(
            [
                os.path.exists(
                    os.path.join(
                        SCRATCH_DIR, protein_foldername, protein + "-model_v3.cif"
                    )
                )
                for protein in proteins
            ]
        ...
        proteins_to_move = [
            os.path.join(SCRATCH_DIR, protein_foldername, protein + "-model_v3.cif")
            for protein in proteins
        ]
        subprocess.run(
            ["mv"] + proteins_to_move + [os.path.join(WORK_PATH, protein_foldername)]
        )
...
def run(random, foldseek_file):
    ...
    get_protein_tm_score(
        os.path.join(WORK_PATH, protein),
        protein + "-model_v3.cif",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--random", type=int, required=False, default=1000)
    parser.add_argument(
        "--candidate-protein-location", type=str, required=False, default=FOLDSEEK_DIR
    )
    parser.add_argument(
        "--output", type=str, required=False, default="protein_distances"
    )

    args = parser.parse_args()

    os.makedirs(args.output, exist_ok=True)
    proteins = os.listdir(args.candidate_protein_location)

    processed_proteins = [
        file.split("-dist")[0]
        for file in os.listdir(args.output)
        if file.endswith("distances.json")
    ]
