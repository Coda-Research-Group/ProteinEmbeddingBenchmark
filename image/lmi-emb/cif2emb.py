...
def _untar(tar_file, input_dir, scratch_dir):
    ...
    subprocess.run(["tar", "-xf", tar_file_path, "-C", scratch_dir, "--wildcards", "*.cif.gz"])
...
def extract(input_dir, scratch_dir, index):
    ...
    print(f'{datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} - Started extrating data tar/gz')
...
def run(pdb_path, output_path, granularity):
    ...
    for i, protein in enumerate(proteins):
        if (i % 100 == 0):
            print(f'{datetime.now().strftime("%m/%d/%Y, %H:%M:%S")} - Processing "{i}-th" protein ({protein})')
...
def create_descriptor(chain_df, granularity):
    ...
    def compute_matrix(row):
        dist =  np.linalg.norm(np.array([row['x_x'], row['y_x'], row['z_x']]) - np.array([row['x_y'], row['y_y'], row['z_y']]))
...
def read_and_extract(protein_file, granularity):
    ...
    df = pd.DataFrame({'atom': atoms, 'residue': residues, 'chain': chains, 'residue_sequence': coded_residue_sequences, 'x': xs, 'y': ys, 'z': zs})
...
if __name__ == "__main__":
    ...
    run(extracted_data, os.path.join(output_path, f'cif2emb-result-{args.position}'), args.granularity)
