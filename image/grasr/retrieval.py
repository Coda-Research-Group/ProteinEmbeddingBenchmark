...
def retrieval(q_descriptors: dict, k_descriptors: dict) -> dict:
    ...
    dist_mat = len_scaling_cos_dist(
        torch.from_numpy(q_des), torch.from_numpy(k_des), q_len, k_len
    )
    ...
    for i, q_id in enumerate(q_ids):
        ...
        for j in range(10):
            if dist_mat[i, order_tensor[i, j]].abs() < 1e-5:
                ...
                pdb_dist_list.append(
                    (k_ids[order_tensor[i, j]], dist_mat[i, order_tensor[i, j]])
                )
...
def len_scaling_cos_dist(
    x1: torch.Tensor, x2: torch.Tensor, len_1: list, len_2: list
) -> torch.Tensor:
    """
    ...
    len_1_t, len_2_t = torch.tensor(len_1, dtype=torch.float), torch.tensor(
        len_2, dtype=torch.float
    )
    max_len = len_2_t.max()
    ...
    len_scaling_mat = 1 + torch.clamp(
        (-len_1_t.unsqueeze(1) + len_2_t.unsqueeze(0)) / max_len, min=0
    )
    ...
    for q_id in retrieval_result.keys():
        with open("{}/{}.txt".format(ret_dir, q_id), "w") as rank_file:
            ...
            for i in range(10):
                rank_file.write(
                    "{}\t\t{:.5f}\n".format(
                        retrieval_result[q_id][i][0], retrieval_result[q_id][i][1]
                    )
                )
