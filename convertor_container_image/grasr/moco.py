...
class MoCo(nn.Module):
    ...
    def _batch_shuffle_ddp(self, x, ld, am):
        ...
        *** Only support DistributedDataParallel (DDP) model. ***
        ...
        seq_len = x.shape[1]
        len_gather = concat_all_gather(
            torch.tensor(seq_len, dtype=torch.int).reshape(-1, 1).cuda()
        )
        max_seq_len = int(len_gather.max())
        x_pad = torch.cat(
            (x, torch.zeros(batch_size_this, max_seq_len - seq_len, x.shape[2]).cuda()),
            dim=1,
        )
        am_pad_row = torch.cat(
            (am, torch.zeros(batch_size_this, seq_len, max_seq_len - seq_len).cuda()),
            dim=2,
        )
        am_pad = torch.cat(
            (
                am_pad_row,
                torch.zeros(batch_size_this, max_seq_len - seq_len, max_seq_len).cuda(),
            ),
            dim=1,
        )
        ...
        return (
            x_pad_gather[idx_this],
            ld_gather[idx_this],
            am_pad_gather[idx_this],
            idx_unshuffle,
        )
...
def concat_all_gather(tensor):
    ...
    *** Warning ***: torch.distributed.all_gather has no gradient.
    """
    tensors_gather = [torch.ones_like(tensor) for _ in range(td.get_world_size())]
