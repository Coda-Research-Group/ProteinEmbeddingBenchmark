...
class DeepCNN(object):
    ...
    def __init__(
        ...
        if lm_model_name is not None:
            ...
            lm_model = tf.keras.Model(
                inputs=lm_model.input, outputs=lm_model.get_layer("LSTM2").output
            )
    ...
    def _build_model(
        ...
        if lm_model is not None:
            x_lm = tf.keras.layers.Dense(128, use_bias=False, name="LM_embedding")(
                lm_model(x)
            )
        ...
        for l in range(0, len(num_filters)):
            x_l = tf.keras.layers.Conv1D(
                filters=num_filters[l],
                kernel_size=filter_lens[l],
                padding="same",
    ...
    def load_model(self):
        self.model = tf.keras.models.load_model(
            self.model_name_prefix, custom_objects={"FuncPredictor": FuncPredictor}
        )
