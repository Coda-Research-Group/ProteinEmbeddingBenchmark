...
from .layers import FuncPredictor, GraphConv, MultiGraphConv, SumPooling
...
class Predictor(object):
    ...
    def predict_from_PDB_dir(self, dir_name, cmap_thresh=10.0):
        ...
        self.chain2path = {
            pdb_fn.split("/")[-1].split(".")[0]: pdb_fn for pdb_fn in pdb_fn_list
        }
        self.test_prot_list = list(self.chain2path.keys())
        self.Y_hat = np.zeros(
            (len(self.test_prot_list), len(self.goterms)), dtype=float
        )
        self.goidx2chains = {}
        self.prot2goterms = {}
        self.data = {}
        for i, chain in enumerate(self.test_prot_list):
            A, S, seqres = self._load_cmap(
                self.chain2path[chain], cmap_thresh=cmap_thresh
            )
            y = self.model([A, S], training=False).numpy()[:, :, 0].reshape(-1)
            self.Y_hat[i] = y
            self.prot2goterms[chain] = []
