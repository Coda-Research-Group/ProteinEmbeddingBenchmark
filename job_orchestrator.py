...
def run_job(job):
    ...
    api_job = client.V1Job(api_version="batch/v1", kind="Job", metadata=manifest.get("metadata"), spec=manifest.get("spec"))
    ...
    try:
        api_instance.read_namespaced_job_status(name=manifest.get('metadata').get('name'), namespace=args.namespace).status
    ...
    while True:
        job_status = api_instance.read_namespaced_job_status(name=manifest.get('metadata').get('name'), namespace=args.namespace)
    ...
    with ThreadPoolExecutor(max_workers=max_concurrent_jobs) as executor:
        ...
        while job_id <= from_job_id + num_jobs - 1:
            ...
            if executor._work_queue.qsize() >= args.max_concurrent_jobs:
                print(f"Waiting for jobs to complete. Jobs todo: {executor._work_queue.qsize()}, wait time: {2**wait_time}")
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--job_template", "-t", help="Path to the jinja2 template file.", required=True)
    argparser.add_argument("--output_log", "-o", help="Path to the output file where the results are written.", default="job_log.txt")
    argparser.add_argument("--start_id", "-i", help="The job id to start from.", type=int, required=True)
    argparser.add_argument("--jobs_number", "-j", help="The number of jobs to run.", type=int, required=True)
    argparser.add_argument("--namespace", "-n", help="The namespace to run the jobs in", required=True)
    argparser.add_argument("--max_concurrent_jobs", "-m", help="The maximum number of concurrent jobs", type=int, default=20)
    argparser.add_argument("--dry-run", "-d", help="If set, the jobs will not be submitted to the cluster", action="store_true")
